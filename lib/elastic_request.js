...
ElasticRequest.count = function ( req, options = { }, callback ) {
  ElasticRequest.countAsync( req, options ).then( count => {
    callback( null, count );
  } ).catch( e => callback( e ) );
};

ElasticRequest.countAsync = async function ( req, options = { } ) {
  ...
  const countMethod = async ( ) => {
    const response = await ElasticRequest.esClient.search( Object.assign( {
...
ElasticRequest.reservedCount = async function (
  ...
  const cacheCountMethod = async ( ) => {
    ...
    await req.redisCacheClient.setAsync( reservationCacheKey, "reserved", "EX", RESERVED_COUNT_TIMEOUT / 1000 );
    // run the count query
    const countResult = await countMethod( );
    // cache the count
    await req.redisCacheClient.setAsync( cacheKey, countResult, "EX", COUNT_CACHE_SECONDS );
    // delete the count reservation
    await req.redisCacheClient.delAsync( reservationCacheKey );
    return countResult;
  };
  ...
  const cachedCount = await req.redisCacheClient.getAsync( cacheKey );
  if ( cachedCount ) {
...
ElasticRequest.waitForReservedCount = async function (
  ...
  const reservationExists = await req.redisCacheClient.getAsync( reservationCacheKey );
...
ElasticRequest.geogridPrecision = ( zoom, offset ) => {
  let precision = 3;
  if ( zoom >= 5 ) { precision = 4; }
  if ( zoom >= 8 ) { precision = 5; }
...
ElasticRequest.geotilePrecision = ( zoom, offset ) => {
  let precision = zoom + 5;
  if ( offset && ( precision - offset ) > 0 ) {
    precision -= offset;
  }
    ...
    agg = {
      zoom1: {
        geotile_grid: {
          ...
          size: 10000,
          precision: req.geotilegrid
            ? ElasticRequest.geotileGridPrecision(
              req.params.zoom, req.query ? Number( req.query.precision_offset ) : null
            ) : ElasticRequest.geotilePrecision(
              req.params.zoom, req.query ? Number( req.query.precision_offset ) : null
            )
        }
      }
    ...
    agg = {
      zoom1: {
        geohash_grid: {
          field: global.config.elasticsearch.geoPointField,
          size: 30000,
          precision: req.geogrid
            ? ElasticRequest.geogridPrecision(
              req.params.zoom, req.query ? Number( req.query.precision_offset ) : null
            ) : ElasticRequest.geohashPrecision(
              req.params.zoom, req.query ? Number( req.query.precision_offset ) : null
            )
        }
      }
