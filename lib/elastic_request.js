...
ElasticRequest.count = function ( req, options = { }, callback ) {
  ...
  cache.get( cacheKey, ( err, value ) => {
    ...
    if ( value ) {
      return void callback( null, { hits: { total: { value } } } );
    }
    ElasticRequest.esClient.search( Object.assign( {
      ...
      body: query
    }, options ), ( errr, response ) => {
      if ( errr ) { return void callback( errr ); }
      // cache the total count of the query for 1 day
      cache.set( cacheKey, response.hits.total.value, 24 * 60 * 60, ( ) => {
