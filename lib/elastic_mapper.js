...
ElasticMapper.csvFromResult = function( req, result ) {
  ...
  var fields_to_map = _.without( req.elastic_query.fields, geoPointField );
  var csvData = _.map( target, function( hit ) {
    var fieldData;
    if( hit.geohash ) {
      fieldData = hit.geohash.hits.hits[ 0 ].fields;
    }
    ...
    if( _.isObject( fieldData[ geoPointField ][ 0 ] )) {
      hash.latitude = fieldData[ geoPointField ][ 0 ].lat;
      hash.longitude = fieldData[ geoPointField ][ 0 ].lon;
    } else {
      var coords = fieldData[ geoPointField ][ 0 ].split( "," );
