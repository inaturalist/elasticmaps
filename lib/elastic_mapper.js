...
ElasticMapper.csvFromResult = ( req, result ) => {
  ...
  const fieldsToMap = _.without( req.query.source.includes || [], geoPointField );
...
ElasticMapper.cacheKeyForReq = req => {
  const query = _.cloneDeep( req.query );
  delete query.source;
  delete query.color;
  delete query.width;
  delete query.callback;
  const sorted = _( query ).toPairs( ).sortBy( 0 ).fromPairs( )
    .value( );
...
ElasticMapper.fetchEntireGeohashGrid = req => {
  ...
  console.log( "Starting query cache" ); // eslint-disable-line no-console
  ...
  ElasticRequest.search( { elastic_query: queryCopy }, ( err, r ) => {
    ...
    _.each( data, d => {
      const xyz = MapGenerator.merc.xyz( [
        d.longitude, d.latitude, d.longitude, d.latitude
      ], req.params.zoom );
      fs.appendFileSync( `${req.precisionCacheDir}/${xyz.minX}.${xyz.minY}.data`,
        `${JSON.stringify( d )}\n` );
      ...
      if ( d.latitude < cellbbox[1] + diffLat ) {
        fs.appendFileSync( `${req.precisionCacheDir}/${xyz.minX}.${xyz.minY + 1}.data`,
          `${JSON.stringify( d )}\n` );
      }
      ...
      if ( d.longitude > cellbbox[2] - diffLng ) {
        fs.appendFileSync( `${req.precisionCacheDir}/${xyz.minX + 1}.${xyz.minY}.data`,
          `${JSON.stringify( d )}\n` );
      }
