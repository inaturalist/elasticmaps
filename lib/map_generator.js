...
MapGenerator.torqueJson = function( queryResult, req, res ) {
  ...
  let returnArray = [ ];
  _.each( queryResult.aggregations.zoom1.buckets, b => {
    const hashInfo = Geohash.decode( b.key );
    let vals = [];
    let dates = [];
    _.each( b.histogram.buckets, hb => {
      vals.push( hb.doc_count );
      dates.push( hb.key - 1 );
    });
    const mercCoords = MapGenerator.merc.px( [hashInfo.lon,hashInfo.lat], req.params.zoom );
    const torqueX = Math.floor( ( Math.abs( mercCoords[0] - mercMin[0] ) / latdiff ) * 256 );
    const torqueY = Math.floor( ( Math.abs( mercCoords[1] - mercMin[1] ) / londiff ) * 256 );
    returnArray.push( {
      x__uint8: torqueX,
      y__uint8: torqueY,
      vals__uint8: vals,
