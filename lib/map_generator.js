...
MapGenerator.bboxFromParams = req => {
  ...
  return MapGenerator.merc.bbox(
    ...
    parseInt( req.params.zoom, 10 ), false, "900913"
  );
};
...
MapGenerator.postgisDatasource = req => {
  const cfg = Object.assign( { }, global.config.database, {
    type: "postgis",
...
MapGenerator.memoryDatasource = ( req, fields, features ) => {
  let memDS;
  if ( features.length > 0 ) {
    const featureStrings = _.map( features, feature => {
      const f = Object.assign( { }, feature );
      if ( req.params.dataType === "geojson" ) {
        const j = JSON.stringify( f.geojson ).replace( /"/g, "\\\"" );
        return [`${f.id},"${j}"`].join( "," );
      }
...
MapGenerator.finishMap = ( req, res, map, layer, features, callback ) => {
  ...
  if ( req.params.dataType === "postgis" ) {
    ...
    const memDS = MapGenerator.memoryDatasource( req, fields, features );
  ...
  if ( req.params.format === "grid.json" ) {
    ...
    const im = new mapnik.Grid( 256, 256, { key: "id" } );
    ...
    const im = new mapnik.Image( global.config.tileSize, global.config.tileSize );
    map.render( im, { scale: global.config.tileSize / 256 }, ( err, img ) => {
      if ( err ) { return callback( err, req, res ); }
      req.tileData = img.encodeSync( );
      return callback( null, req, res );
    } );
...
MapGenerator.createMapTemplate = ( req, callback ) => {
  try {
    ...
    const tileSize = ( req.params.format === "grid.json" )
